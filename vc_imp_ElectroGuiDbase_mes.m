...
try
    ...
    for n = 1:length(P.FileRange)
        ...
        % Filter raw signal to remove low freq noise -Stetner
        params = egf_FIRBandPass('params');
        params.Values{1} = '500';
        params.Values{2} = '20000';
        sig = egf_FIRBandPass(sig, dbase.Fs, params);
...
%         end
        ...
        for(ne = 1:length(startNdx))
            ...
            % filter audio to remove low freq noise -Stetner
            ...
            [pi, pg, hp, pt, ent, p, f] = estimatePitch(syllAudio, dbase.Fs); %%% Tatsuo
            ...
            % new features added by Stetner
            handles.scalar_features(nVect,11) = ismember(ne,bSel); %syllable is selected
            
            Pitch{nVect,1} = pi;
            PitchGoodness{nVect,1} = pg;
            Entropy{nVect,1} = ent; 
            [features labels] = egf_SAPfeatures(syllAudio, dbase.Fs); % written by Sigal Saar  
            AM{nVect,1} = features{1};
            FM{nVect,1} = features{2};
            % entropy is features{3}, but we already got that from
            % estimatePitch
            Amplitude{nVect,1} = features{4};
            % 5 and 6 are pitch goodess and pitch
            Pitch_chose{nVect,1} = features{7};
            Pitch_weight{nVect,1} = features{8};
            Mean_frequency{nVect,1} = features{9}; % also known as gravity center
            
            handles.scalar_features(nVect,12) = vc_feat_threshtime(FM{nVect,1}, 50, 20);
        end
    end    
    handles.scalar_feature_names = {'duration';'mean_pitch';'std_pitch';...
        'mean_pitchGoodness';'std_pitchGoodness';...
        'mean_entropy';'std_entropy';'timeInFile';'isi';'interval';...
        'is_selected';'highFM_time'}; %%% changed to column vector, Tatsuo
    handles.vector_features = {Pitch,PitchGoodness,Entropy,FM,...
        Mean_frequency,AM,Amplitude,Pitch_chose,Pitch_weight}; %%% Tatsuo
    handles.vector_feature_names = {'pitch';'pitchGoodness';'entropy'; ...
        'FM';'Mean_frequency';'AM';'Amplitude';'Pitch_chose';'Pitch_weight'};  %%% Tatsuo
