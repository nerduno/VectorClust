function varargout = vc_imp_ProcessedAnnotationFiles(varargin)
% Usages:
%
% GUI MODE
% [v, sf, vf, c, t, i, sf_names, vf_names, fileName, pathName] = vc_imp_ProcessedAnnotationFiles()
%       v: vectors
%       sf: scalar features
%       vf: vector features
%       c: initial cluster 
...
% parameters = vc_imp_ProcessedAnnotationFiles('parameters')
%   returns structure containing a name field.
%
% BATCH MODE 
% [v, sf, vf, c, t, i, sf_names, vf_names] = vc_imp_ProcessedAnnotationFiles('batch', searchString)

% VC_IMP_PROCESSEDANNOTATIONFILES M-file for vc_imp_ProcessedAnnotationFiles.fig
%      VC_IMP_PROCESSEDANNOTATIONFILES by itself, creates a new VC_IMP_PROCESSEDANNOTATIONFILES or raises the
%      existing singleton*.
%
%      H = VC_IMP_PROCESSEDANNOTATIONFILES returns the handle to a new VC_IMP_PROCESSEDANNOTATIONFILES or the handle to
%      the existing singleton*.
%
%      VC_IMP_PROCESSEDANNOTATIONFILES('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in VC_IMP_PROCESSEDANNOTATIONFILES.M with the given input arguments.
%
%      VC_IMP_PROCESSEDANNOTATIONFILES('Property','Value',...) creates a new VC_IMP_PROCESSEDANNOTATIONFILES or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before vc_imp_ProcessedAnnotationFiles_OpeningFunction gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to vc_imp_ProcessedAnnotationFiles_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help vc_imp_ProcessedAnnotationFiles
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @vc_imp_ProcessedAnnotationFiles_OpeningFcn, ...
                   'gui_OutputFcn',  @vc_imp_ProcessedAnnotationFiles_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before vc_imp_ProcessedAnnotationFiles is made visible.
function vc_imp_ProcessedAnnotationFiles_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to vc_imp_ProcessedAnnotationFiles (see VARARGIN)
...
function varargout = vc_imp_ProcessedAnnotationFiles_OutputFcn(hObject, eventdata, handles)
...
handles.fileName = fn; %%% TO
handles.pathName = pn; %%% TO
if(~isempty(fn))
    strs = handles.strs;
    for(i = 1:length(strs))
...
[pathstr] = fileparts(searchString); 
d = dir(searchString);
bSuccess = false;
